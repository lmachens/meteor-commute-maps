window.InitInvertedCircle = function() {
  ...
  */
  ...
  InvertedCircle.prototype.triggerPositionChangedEvent = function() {
    this.get('position_changed_event').call(this, this.get('position'));
  }
  ...
  InvertedCircle.prototype.addCenter_ = function() {
    ...
    this.addressOverlay.setContent(
    ...
    this.addressOverlay.open(this.get('map'), center_marker);
    ...
    center_marker.addListener('click', function() {
      ...
      $('#addressForm input').geocomplete().bind(
        'geocode:result',
        function (e, result) {
          me.setCenter(result.geometry.location, true);
          me.get('map').setCenter(result.geometry.location);
          me.triggerPositionChangedEvent();
          me.addressOverlay.close();
          if (result.geometry.viewport) {
            me.get('map').fitBounds(result.geometry.viewport)
          }
        }
      );
